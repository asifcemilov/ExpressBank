...
class YearsDropdownAdapter(context: Context, private val items: List<Year>) :
    ArrayAdapter<Year>(context, android.R.layout.simple_list_item_1, items) {

    @SuppressLint("ViewHolder")
    override fun getView(position: Int, convertView: View?, parent: ViewGroup): View {
        val binding =
            DropdownItemViewBinding.inflate(LayoutInflater.from(parent.context), parent, false)
        binding.dropdownItemText.text = items[position].year
        return binding.root
    }
